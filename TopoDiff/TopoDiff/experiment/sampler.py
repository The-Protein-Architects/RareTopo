...
class Sampler:
    ...
    def _init_model(self):
        ...
        if self.sample_config.get('lora', False):
            target_keywords = [target.strip() for target in self.sample_config.get('lora_target', 'backbone').split(',') if target.strip()]
            replaced = apply_lora_to_linear(
                self.model_diffusion,
                target_keywords=target_keywords,
                r=self.sample_config.get('lora_rank', 8),
                alpha=self.sample_config.get('lora_alpha', 16),
                dropout=self.sample_config.get('lora_dropout', 0.0),
            )
            logger.info('Applied LoRA before loading diffusion checkpoint, modules replaced: %d', len(replaced))
        ...
        print("num unexpected:", len(unexpected_keys))
        ...
        packed_latent_lora = ckpt.get('latent_lora_config', {})
        use_latent_lora = self.sample_config.get('latent_lora', False) or packed_latent_lora.get('enabled', False)
        if use_latent_lora:
            latent_lora_rank = self.sample_config.get('latent_lora_rank', 4)
            latent_lora_alpha = self.sample_config.get('latent_lora_alpha', 8)
            latent_lora_target = self.sample_config.get('latent_lora_target', 'backbone')
            if packed_latent_lora.get('enabled', False) and not self.sample_config.get('latent_lora', False):
                latent_lora_rank = packed_latent_lora.get('rank', latent_lora_rank)
                latent_lora_alpha = packed_latent_lora.get('alpha', latent_lora_alpha)
                latent_lora_target = packed_latent_lora.get('target', latent_lora_target)
            target_keywords = [
                target.strip()
                for target in latent_lora_target.split(',')
                if target.strip()
            ]
            replaced = apply_lora_to_linear(
                self.model_latent,
                target_keywords=target_keywords,
                r=latent_lora_rank,
                alpha=latent_lora_alpha,
                dropout=self.sample_config.get('latent_lora_dropout', 0.0),
            )
            if not replaced:
                raise ValueError('No Linear modules matched latent LoRA target(s): %s' % target_keywords)
            logger.info('Applied latent LoRA before loading checkpoint, modules replaced: %d', len(replaced))
