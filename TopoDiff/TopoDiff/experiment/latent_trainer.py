...
from TopoDiff.model.lora import (
...
def get_latent_train_config(args):
    ...
    train_cfg.Base.max_epochs = args.n_epoch if args.n_epoch is not None else default_latent_training_config['Base']['max_epochs']
    ...
    train_cfg.Data.batch_size = args.latent_batch_size
    if args.latent_lr is not None:
        train_cfg.Strategy.lr = args.latent_lr
...
class MyLatentTrainer():
    def __init__(self, args, rank, world_size, log = False):
        ...
        self.output_dir = self.train_config.Base.root_dir
    ...
    def _init_model(self):
        model = LatentDiffusion(self.config, log = self.log)
        checkpoint = None
        checkpoint_has_lora = False

        def apply_lora(model):
            target_keywords = [target.strip() for target in self.args.lora_target.split(',') if target.strip()]
            logger.info(
                'Applying latent LoRA: target=%s, rank=%d, alpha=%d, dropout=%.4f',
                target_keywords,
                self.args.lora_rank,
            ...
            if not replaced:
                raise ValueError('No Linear modules matched the latent LoRA target(s): %s' % target_keywords)
            logger.info('Latent LoRA modules replaced: %d', len(replaced))
        ...
        if self.train_config.Base.model_load_path is not None:
            ...
            checkpoint = torch.load(self.train_config.Base.model_load_path, map_location='cpu', weights_only=False)
            if isinstance(checkpoint, dict) and 'latent_ckpt' in checkpoint:
                checkpoint = checkpoint['latent_ckpt']
            elif isinstance(checkpoint, dict) and 'state_dict' in checkpoint:
                checkpoint = checkpoint['state_dict']

            if not isinstance(checkpoint, dict):
                raise ValueError('Unsupported latent checkpoint format: {}'.format(type(checkpoint)))

            checkpoint_has_lora = any('lora_A' in key or 'lora_B' in key for key in checkpoint.keys())
            if getattr(self.args, 'lora', False) and checkpoint_has_lora:
                apply_lora(model)
        ...
        if getattr(self.args, 'lora', False):
            if not checkpoint_has_lora:
                apply_lora(model)
            mark_only_lora_as_trainable(model)
            trainable, total = count_trainable_parameters(model)
            logger.info('Latent trainable params: %d / %d (%.4f%%)', trainable, total, 100 * trainable / total)
    ...
    def _init_optimizer_and_scheduler(self):
        ...
        if len(trainable_params) == 0:
            raise ValueError('No trainable parameters found for latent optimizer')
    ...
    def run(self):
        progress_bar = tqdm(
            total=self.train_config.Base.max_epochs,
    ...
    def train_step_end(self, batch_part):
        ...
        if self.rank == 0 and self.tb_writer is not None:
            # 1) record the scalar loss actually used for backprop
            self.tb_writer.add_scalar(
                "train/total_loss",
                batch_part['loss'].item(),
                self.global_step
            )

            # 2) if loss_dict has recon, record it separately
            if 'recon' in loss_dict:
                self.tb_writer.add_scalar(
                    "train/recon",
                    float(loss_dict['recon'][0]),
                    self.global_step
                )

            # 3) additional loss_dict entries can be logged here
            if 'total_loss' in loss_dict:
                self.tb_writer.add_scalar(
                    "train/total_loss_dict",
                    float(loss_dict['total_loss'][0]),
                    self.global_step
                )
    ...
    def _pack_ckpt(self):
        if self.rank == 0:
            ...
            ckpt_dict['latent_lora_config'] = {
                'enabled': bool(getattr(self.args, 'lora', False)),
                'rank': int(getattr(self.args, 'lora_rank', 4)),
                'alpha': int(getattr(self.args, 'lora_alpha', 8)),
                'target': str(getattr(self.args, 'lora_target', 'backbone')),
            }
