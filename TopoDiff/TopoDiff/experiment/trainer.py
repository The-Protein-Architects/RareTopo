...
default_training_config = {
    "Base" : {
        ...
        "ckpt_save_interval" : 5, #25,
        'ckpt_state_save_interval': 5,  #25,
        "log_to_main_interval" : 5,  #25,
...
def get_train_config(args):
    ...
    train_cfg.Base.max_epochs = args.n_epoch if args.n_epoch is not None else 1000
    ...
    if args.stage == 1:
        ...
        train_cfg.Data.train_data_dir = args.train_data_dir or os.path.join(project_dir, 'my_train_data', 'pt_data', 'data')
        train_cfg.Data.train_data_cache_path = args.train_data_cache_path or os.path.join(project_dir, 'my_train_data', 'my_train_json.json')
...
class MyTrainer():
    ...
    def _init_model(self):
        ...
        def apply_lora(model):
            ...
            logger.info(
                'Applying LoRA: target=%s, rank=%d, alpha=%d, dropout=%.4f',
                target_keywords,
                self.args.lora_rank,
            ...
            logger.info('LoRA modules replaced: %d', len(replaced))
        ...
        if self.train_config.Base.model_load_path is not None:
            ...
            checkpoint = torch.load(self.train_config.Base.model_load_path, map_location = 'cpu')
            if isinstance(checkpoint, dict) and 'main_ckpt' in checkpoint:
                checkpoint = checkpoint['main_ckpt']
        ...
        if getattr(self.args, 'lora', False):
            ...
            logger.info('Trainable params: %d / %d (%.4f%%)', trainable, total, 100 * trainable / total)
