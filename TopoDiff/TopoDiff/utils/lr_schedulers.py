...
class TopoDiffLRScheduler(torch.optim.lr_scheduler._LRScheduler):
    """ Implements the learning rate schedule defined in the AlphaFold 2
        ...
        from OpenFold's implementation
    """
    def __init__(self, 
        optimizer, 
        last_epoch: int = -1, 
        ...
        base_lr: float = 0.,
        max_lr: float = 5e-4,
        warmup_no_steps: int = 500,  # 1000
        start_decay_after_n_steps: int = 500,
        ...
        super(TopoDiffLRScheduler, self).__init__(
            optimizer,
