...
def cast_to_64bit_ints(protein):
    """Adapted from OpenFold's cast_to_64bit_ints function.
    """
    for k, v in protein.items():
        if isinstance(v, torch.Tensor) and v.dtype == torch.int32:
            protein[k] = v.type(torch.int64)

    return protein

def squeeze_features(protein):
    """Adapted from OpenFold's squeeze_features function.
    """
    if "aatype" in protein:
        protein['aatype'] = torch.argmax(protein["aatype"], dim=-1)
    else:
        protein['aatype'] = protein['seq_type']
    for k in [
        "domain_name",
        "msa",
        "num_alignments",
        ...
        if k in protein and protein[k].ndim > 0:
...
def make_seq_mask(protein):
    """Adapted from OpenFold's make_seq_mask function.
    """
...
def make_atom14_masks(protein):
    """Adapted from OpenFold's make_atom14_masks function.
    """
...
def make_atom14_positions(protein):
    """Adapted from OpenFold's make_atom14_positions function.
    """
...
def atom37_to_frames(protein, eps=1e-8):
    """Adapted from OpenFold's atom37_to_frames function.
    """
...
def atom37_to_torsion_angles(protein, prefix=""):
    """
    Adapted from OpenFold's atom37_to_torsion_angles function.
...
def get_backbone_frames(protein):
    protein["frame_gt"] = protein["rigidgroups_gt_frames"][
        ..., 0, :, :
    ]

    protein["frame_gt_mask"] = protein["rigidgroups_gt_exists"][..., 0]
...
@curry1
def make_contact(protein, config = None):
    if config is None or config.contact_type == "CA":
        ca_idx = rc.atom_order["CA"]
        protein['contact_gt'] = protein["frame_gt"][..., :3, -1]
        protein['contact_gt_mask'] = protein['all_atom_mask'][..., ca_idx]
    elif config.contact_type == "pseudo_beta":
        protein['contact_gt'], protein['contact_gt_mask'] = pseudo_beta_fn(
            protein["aatype"],
            protein["all_atom_positions"],
            protein["all_atom_mask"],
        )
    else:
        raise ValueError("Unknown contact type: {}".format(config.contact_type))
    return protein
